      ...
      <div className="relative z-10 mx-auto max-w-5xl px-4 py-20 md:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <PriceCard
            ...
            CTA={<GhostButton className="w-full text-black">Get started free</GhostButton>}
            benefits={[
          ...
          <PriceCard
            ...
            CTA={
              <GhostButton className="w-full bg-zinc-950 text-white hover:bg-zinc-900 hover:text-black">
                14-day free trial
              </GhostButton>
            }
            benefits={[
  ...
  return (
    <Card>
      <div className="flex flex-col items-center border-b border-black pb-6">
        <span className="mb-6 inline-block text-black">{tier}</span>
...
const Benefit = ({ text, checked }: BenefitType) => {
  return (
    <div className="flex items-center gap-3">
      {checked ? (
        <span className="grid size-5 place-content-center rounded-full bg-blue-600 text-sm text-black">
          <FiCheck />
        </span>
      ) : (
        <span className="grid size-5 place-content-center rounded-full bg-zinc-800 text-sm text-black">
          <FiX />
        </span>
      )}
...
const Card = ({ className, children, style = {} }: CardProps) => {
  return (
    <motion.div
      ...
      style={style}
      className={twMerge(
        "relative h-full w-full overflow-hidden rounded-2xl border border-black bg-gradient-to-br from-zinc-950/50 to-zinc-900/80 p-6",
        className
      )}
    >
      {children}
